...
export default function BusetasListScreen() {
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = busetas.filter(
      (buseta) => buseta.placa.toLowerCase().includes(trimmed)
    );
  ...
  const renderEmpty = () => (
    <View className="items-center justify-center py-12">
      ...
      <Text className="text-lg font-semibold text-gray-700 mb-2">
        {searchQuery ? 'No se encontraron busetas' : 'No hay busetas registradas'}
      </Text>
      ...
      <View className="bg-buseta-700 pb-6 px-6 rounded-b-3xl" style={[{ paddingTop }, shadow]}>
        <View className="flex-row items-center justify-between mb-4">
          ...
          <TouchableOpacity
            ...
            className="bg-buseta-600 px-4 py-2 rounded-lg flex-row items-center"
          >
        ...
        <View className="flex-row items-center">
          ...
          <View className="flex-1">
            <Text className="text-white text-2xl font-bold">Busetas</Text>
